...
export default function BlogPage() {
  ...
  return (
    <div className="bg-background">
      {/* Hero */}
      <div className="bg-gradient-to-br from-brand-600 to-brand-800 text-center px-[5%] py-14">
      ...
      <div className="max-w-[1100px] mx-auto px-[5%] py-12">
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {CATEGORIAS.map((c, i) => (
            <span key={c} className={`text-sm px-4 py-1.5 rounded-full font-medium cursor-pointer transition-colors ${i === 0 ? 'bg-brand-600 text-white' : 'bg-card border border-border text-muted-foreground hover:border-brand-200 hover:text-foreground'}`}>
              {c}
            </span>
          ))}
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden mb-10 flex flex-col lg:flex-row">
          <div className="w-full lg:w-64 flex-shrink-0 bg-gradient-to-br from-brand-600 to-brand-800 py-12 lg:py-0">
          </div>
          <div className="p-7 flex flex-col justify-center">
            <span className="text-xs font-semibold text-brand-400 uppercase tracking-wider mb-3">{FEATURED.categoria}</span>
            <h2 className="text-xl font-bold text-foreground leading-snug mb-3 hover:text-brand-600 dark:hover:text-brand-300 transition-colors cursor-pointer">{FEATURED.titulo}</h2>
            <p className="text-sm text-muted-foreground leading-relaxed mb-5">{FEATURED.extracto}</p>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-brand-600 flex items-center justify-center text-xs font-bold text-white">{FEATURED.autorIniciales}</div>
              <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {POSTS.map(post => (
            <div key={post.id} className="bg-card border border-border rounded-2xl p-6 hover:border-brand-300 hover:shadow-[0_4px_20px_rgba(74,31,168,.09)] transition-all group cursor-pointer">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-brand-400">{post.categoria}</span>
              <h3 className="text-base font-bold text-foreground mt-2 mb-2 group-hover:text-brand-600 dark:group-hover:text-brand-300 transition-colors leading-snug">{post.titulo}</h3>
        ...
        {/* Newsletter */}
        <div className="mt-14 bg-gradient-to-br from-brand-600 to-brand-800 rounded-2xl p-8 text-white text-center">
          ...
          {subscribed ? (
            ...
            <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto" onSubmit={e => { e.preventDefault(); setSubscribed(true) }}>
              ...
              <button type="submit" className="bg-white text-brand-600 font-semibold text-sm px-6 py-2.5 rounded-xl hover:bg-brand-50 transition-colors flex-shrink-0">
                Suscribirse
              </button>
            </form>
          )}
